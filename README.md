...
- 찜 목록은 사용자의 브라우저 `localStorage`에 저장되어 로그인 없이 사용할 수 있습니다.
- 배포 환경의 댓글은 Netlify Blobs에 장소별로 영구 저장됩니다.
- 로컬 실행의 댓글은 Git에서 제외된 `backend/comments.local.json`에 저장됩니다.
- 댓글 수정과 삭제에는 작성할 때 정한 비밀번호가 필요합니다.
- 비밀번호는 원문으로 저장하지 않고 서버에서 `scrypt` 해시와 임의 salt로 보호하며 API 응답에도 포함하지 않습니다.
- 작성자 2~30자, 댓글 2~500자, 비밀번호 4~50자로 제한하고 장소 ID를 실제 TourAPI 데이터와 대조합니다.
- Netlify Blobs의 일관성 특성상 새 댓글은 화면에 즉시 표시되지만 다른 요청의 수정·삭제에는 수십 초의 전파 시간이 필요할 수 있습니다.

Netlify 배포에는 공식 `@netlify/blobs` 패키지를 사용하므로 루트의 `package.json`과 `package-lock.json`을 함께 배포해야 합니다.
...
배포 후에는 다음 명령으로 검색·상세·지도·찜·댓글·날씨·챗봇을 한 번에 확인할 수 있습니다.

```bash
node scripts/verify-live.mjs https://stately-begonia-1aaa4b.netlify.app
```
