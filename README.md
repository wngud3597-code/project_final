...
지방에 계신 부모님이 서울을 방문할 때 편하게 사용할 수 있도록 만든 **관광 검색 + 상세정보 + 지도 + 기상청 날씨** 통합 서비스입니다.
...
- 찜한 장소 브라우저 저장
- 보통 글씨 / 큰 글씨 전환
- 부모님도 편한 큰 입력창, 큰 버튼, 높은 명도 대비, 키보드 포커스
- 모바일 반응형
- 데이터 출처와 라이선스 표기
...
기본 설정은 `CHAT_MODE=rules`인 무료 스마트 안내 모드입니다. OpenAI API 키 없이 LocalHub 관광지 원본과 기상청 날씨만으로 지역·유형·실내외·동행 조건을 분석해 최대 3곳을 추천합니다.

화면 오른쪽 아래의 **스마트 관광 안내** 버튼은 별도 키나 비용 없이 바로 사용할 수 있습니다. OpenAI API 연동은 원하는 경우에만 선택적으로 활성화할 수 있습니다.

```env
OPENAI_API_KEY=발급받은_API_키
# 선택 사항 (기본값: gpt-5.6-terra)
OPENAI_MODEL=gpt-5.6-terra
```

API 키는 프런트엔드로 전달되지 않으며 LocalHub Python 백엔드에서만 읽습니다. 챗봇은 사용자의 질문과 일치하는 로컬 TourAPI 관광지 후보를 먼저 선별하고, 기상청 키가 설정되어 있으면 후보 지역의 실제 날씨도 함께 사용해 추천합니다. 운영시간·요금처럼 데이터에 없는 정보는 임의로 만들지 않도록 제한되어 있습니다.
