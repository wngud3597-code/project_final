...
2026-07-15 기준으로 공개 페이지, 관광 데이터 6,518건, 조건 검색, 상세정보 API와 무료 스마트 챗봇의 외부 접속 검증을 완료했습니다.

## 준비된 배포 구조

- `frontend/`: Vue 3 화면
- `netlify/functions/api.mjs`: 검색, 상세정보, 지도, 기상청 날씨, 무료 스마트 챗봇 API
...
키를 추가한 뒤 새 배포를 실행합니다. 키는 브라우저나 GitHub에 노출되지 않습니다. 키가 없어도 관광 검색·지도·무료 챗봇은 정상 작동하며 날씨만 사용할 수 없습니다.
...
`/api/health` 응답의 `status`가 `ok`, `loadedItems`가 `6518`, `chatMode`가 `free-rules-netlify`이면 정상입니다.
